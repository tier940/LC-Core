...
- **World class InnerClasses attribute corruption fixed**
    - Coremods using `ClassWriter(COMPUTE_MAXS)` corrupted the `InnerClasses` attribute on `net.minecraft.world.World`, causing `NoClassDefFoundError: World$2` crashes on startup. The transformer now uses exact class matching only and recomputes frames properly.

- **VerifyError caused by over-eager transformer matching**
    - The `WorldInnerClassFixTransformer` was matching any class name containing "World", which corrupted unrelated classes like `WorldPlugin` and `WorldWorkerManager`. It now matches only `net.minecraft.world.World` exactly.
